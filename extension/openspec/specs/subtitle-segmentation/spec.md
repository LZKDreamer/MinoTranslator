...
安全网扫描规则：
- 遍历最终 sentences 数组，对每个不以 `.?!。？！` 结尾的句子 `S[i]`：
...
系统 SHALL 在安全网扫描结束后，扫描最终数组，对仍不以 `.?!。？！` 结尾的句子输出 WARN 级 `debugLog`（`'segmentSentences: incomplete sentence remains at #N: "..."'`），但不中断处理。

安全网 MUST NOT 改变以 `.?!。？！` 结尾的完整句。
...
#### Scenario: 半句话被安全网合并
 ...
 THEN 安全网检测到 `S[i]` 不以 `.?!。？！` 结尾、gap≈0.5s < 1s、合并 duration≈13.5s < 15s、gap < 2s
...
#### Scenario: gap 超过 1s 不合并
 ...
 WHEN `S[i]` 不以句末标点结尾，`S[i+1]` 存在，但 gap = 1.5s（语义停顿）
...
#### Scenario: 词级 json3 数据安全网不误触发
 ...
 THEN 安全网扫描时大多数句子以标点结尾或 gap > 1s，触发率极低
...
1. 优先选择 `sent[q].text` 以 `.?!。？！` 结尾的 q（q < len-1），在 q 之后切分。若有多个标点切分点，选**最接近句子中点**的那个（平衡左右半长度）。
...
#### Scenario: 长句无标点退化到最大间隔
 ...
 WHEN 句子 15s，6 词，整段无 `.?!。？！`（罕见，ASR 漏标点）
