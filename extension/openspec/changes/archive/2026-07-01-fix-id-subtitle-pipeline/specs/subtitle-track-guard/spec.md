...
The verification SHALL use `detectSourceLanguage()` on a sample of the first 10 subtitle events' text. The detected language MUST match the expected canonical language key (via `resolveToLangCode`).
...
- **THEN** `detectSourceLanguage()` on the sampled text returns `'id'`
- **AND** `resolveToLangCode('id').key === resolveToLangCode('id').key` → verification passes
...
- **THEN** `detectSourceLanguage()` returns `'en'` (not `'id'`)
- **AND** verification fails → the text is rejected
- **AND** `fetchSubtitleFile` logs a warning and continues to the next InnerTube client

#### Scenario: Auto mode skips verification

- **WHEN** `preferredLang` is `'auto'`
- **THEN** content language verification is skipped entirely
- **AND** text is returned from any client without language checks
