...
- **智能语言匹配**: `auto` 模式优先匹配视频音频语言 (`audioTracks`)，再回退首条字幕；手动模式按用户选择精确匹配 `captionTracks`
- **源语言选择**: 用户可手动指定源语言（11 种），不存在的语言自动降级并 Toast 提示
- **多语言缓存**: 复合 key `videoId:targetLanguage`，同一视频支持多语言翻译并存
- **Toast 反馈**: 设置变更、缓存清除时有 Toast 提示，源语言匹配失败自动恢复
...
- 视频任务列表（缩略图 + 圆形进度环 + 标题 + 语言对标签`en→zh-CN` + 操作按钮）
- 状态对应操作：翻译 / 取消 / 重试 / 打开
- 源语言选择器（11 种语言，默认 `auto` 智能匹配）
- 目标语言选择器（10 种语言，默认 `auto` 自动检测浏览器语言）
- 打开完整设置页
- 自动轮询（1.5s 间隔）刷新任务列表
- Toast 通知：设置变更提示、源语言匹配失败提示
...
- **语言支持**: 源语言 11 种 + auto，目标语言 10 种 + auto（浏览器语言检测）
- **划词长度限制**: 最多 2000 字符
